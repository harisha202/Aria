...
class FeedbackEmailTemplates:
    ...
    def feedback_to_team(
        ...
        body = f"""
<div style="font-size:20px; font-weight:700; color:#5b21b6; margin-bottom:14px;">New ARIA feedback</div>
<div style="background:#f5f3ff; border:1px solid #ede9fe; border-radius:8px; padding:16px; margin:18px 0 22px;">
    <div style="font-size:14px; color:#4c1d95; margin-bottom:10px;"><strong>Name:</strong> {safe_name}</div>
    <div style="font-size:14px; color:#4c1d95; margin-bottom:10px;"><strong>Email:</strong> {safe_email}</div>
</div>
<div style="background:#f5f3ff; border:1px solid #ede9fe; border-radius:8px; padding:18px; margin-top:22px;">
    <div style="font-size:15px; line-height:1.7; color:#4c1d95;">{safe_message}</div>
</div>
"""
        return base_email(
            "ARIA - New Feedback",
    ...
    def feedback_confirmation(
        ...
        body = f"""
<div style="font-size:20px; font-weight:700; color:#0369a1; margin-bottom:14px;">Thanks, {safe_name}.</div>
<div style="font-size:15px; line-height:1.75; color:#0c4a6e; margin-bottom:24px;">
    Your feedback reached the ARIA team. We read every note because it helps shape a better voice-first assistant.
</div>
<div style="background:#f0f9ff; border:1px solid #e0f2fe; border-radius:8px; padding:18px;">
    <div style="font-size:14px; line-height:1.7; color:#0c4a6e;">{safe_message}</div>
</div>
"""
        return base_email(
